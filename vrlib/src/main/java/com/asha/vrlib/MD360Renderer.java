...
public class MD360Renderer implements GLSurfaceView.Renderer {
	...
	public void onDrawFrame(GL10 glUnused){
		...
		for (int i = 0; i < size; i++){
			...
			//MDRay ray = VRUtil.point2Ray(width >> 1, height >> 1, directors.get(i));
			//MDVector3D orig = ray.getOrig();
			//MDVector3D dir = ray.getDir();
			//Log.i("ori", String.format("x: %4f y: %4f z: %4f", orig.getX(), orig.getY(), orig.getZ()));
			//Log.i("dir", String.format("x: %4f y: %4f z: %4f", dir.getX(), dir.getY(), dir.getZ()));

			MD360Director director = directors.get(i);
			float[] viewMatrix = director.getViewMatrix();
			//Log.i("director", String.format("x: %5f, y: %5f, z:%5f", -1*viewMatrix[9], -1*viewMatrix[10], -1*viewMatrix[11]));
			Log.i("director", String.format("x: %5f, y: %5f, z:%5f", director.getPreviousX(), director.getPreviousY(), 0.0));
